...
const Footer = () => {
    ...
    const linkSections = [
        ...
        },
        {
            ...
            links: [
                ...
                { text: "Become a Plus member", path: '/pricing', icon: null },
                { text: "Open your store", path: '/create-store', icon: null },
            ]
        },
    ];
    ...
    return (
        <footer className="bg-cream-50 border-t border-ink-200 mt-10">
            <div className="max-w-7xl mx-auto px-6">
                {/* Top: wordmark + tagline + link columns, arranged as a single row of columns */}
                <div className="grid md:grid-cols-4 gap-10 py-16">
                    <div className="md:col-span-2 pr-6">
                        <Link href="/" className="font-display text-3xl text-ink-800">
                            e-<span className="text-clay-500">bazaar.</span>
                        </Link>
                        <p className="max-w-sm mt-5 text-sm text-ink-500">
                            A carefully-stocked marketplace for the latest and smartest gadgets — smartphones, smartwatches, and the accessories worth owning.
                        </p>
                        <div className="flex items-center gap-3 mt-6">
                            {socialIcons.map((item, i) => (
                                <Link href={item.link} key={i} className="flex items-center justify-center w-9 h-9 border border-ink-200 hover:border-clay-400 transition rounded-full">
                                    <item.icon />
                                </Link>
                            ))}
                        </div>
                    ...
                    {linkSections.map((section, index) => (
                        <div key={index}>
                            <h3 className="text-xs uppercase tracking-[0.2em] text-ink-400 mb-4">{section.title}</h3>
                            <ul className="space-y-2.5 text-sm text-ink-600">
                                {section.links.map((link, i) => (
                                    <li key={i}>
                                        <Link href={link.path} className="hover:text-clay-600 transition">{link.text}</Link>
                                    </li>
                                ))}
                            </ul>
                        </div>
                    ...
                    <div>
                        <h3 className="text-xs uppercase tracking-[0.2em] text-ink-400 mb-4">Reach us</h3>
                ...
                {/* Bottom bar */}
                <div className="flex flex-col sm:flex-row items-center justify-between gap-2 py-6 border-t border-ink-200 text-xs text-ink-400">
                    <p>© 2025 e-bazaar. All rights reserved.</p>
                    <p>Made for people who like their tech simple.</p>
                </div>
            </div>
        </footer>
    );
